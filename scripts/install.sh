...
if [[ -z "${V_ADMIN_PASSWORD}" ]]; then
    echo "Enter a password for Tino's dashboard:"
...
server {
    ...
    listen [::]:80;
    server_name _;
    root /home/${V_USERNAME}/tino/public;
...
mkdir /home/${V_USERNAME}/tino
chown -R ${V_USERNAME}:${V_USERNAME} /home/${V_USERNAME}/tino
chmod -R 755 /home/${V_USERNAME}/tino
rm /etc/nginx/sites-available/default
rm /etc/nginx/sites-enabled/default
echo "${V_VHOST_CONFIG}" | tee /etc/nginx/sites-available/tino
ln -s /etc/nginx/sites-available/tino /etc/nginx/sites-enabled/
...
php artisan user:create Tino ${V_ADMIN_EMAIL} ${V_ADMIN_PASSWORD}
openssl genpkey -algorithm RSA -out /home/${V_USERNAME}/tino/storage/ssh-private.pem
chmod 600 /home/${V_USERNAME}/tino/storage/ssh-private.pem
ssh-keygen -y -f /home/${V_USERNAME}/tino/storage/ssh-private.pem > /home/${V_USERNAME}/tino/storage/ssh-public.key
chown -R ${V_USERNAME}:${V_USERNAME} /home/${V_USERNAME}/tino/storage/ssh-private.pem
chown -R ${V_USERNAME}:${V_USERNAME} /home/${V_USERNAME}/tino/storage/ssh-public.key

# setup supervisor
export V_WORKER_CONFIG="
[program:worker]
process_name=%(program_name)s_%(process_num)02d
command=php /home/${V_USERNAME}/tino/artisan queue:work --sleep=3 --backoff=0 --queue=default,ssh,ssh-long --timeout=3600 --tries=1
autostart=1
autorestart=1
user=tino
...
# setup cronjobs
echo "* * * * * cd /home/${V_USERNAME}/tino && php artisan schedule:run >> /dev/null 2>&1" | sudo -u ${V_USERNAME} crontab -
